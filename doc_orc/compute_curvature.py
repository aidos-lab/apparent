...
if __name__ == "__main__":
    ...
    new_column_label = format_column_labels(args)

    df = pd.read_pickle(config.DATA_PATH + "nx_networks_undirected_local_hsa.pkl")
    ...
    df = df.assign(
        **{new_column_label: df.G.swifter.apply(
            lambda row: calculate_curvature(row, args.curvature, alpha=args.alpha, prob_fn=args.prob_fn)
        )}
    )
    ...
    print("----------------------------------")
    print(f"New Column Added: {new_column_label}")
